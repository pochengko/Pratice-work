...
for i in range(0,36,9):
    ...
    select_sql="select * from pc_voc where `PublishTime`= %s and `SiteName`= %s "
    cursor.execute(select_sql,(PublishTime,SiteName,))
    result=cursor.fetchall()
    conn.commit()
    SiteName = SiteName
    if result==():
        insert_sql="insert into pc_voc(id,SiteName,Humidity,Illuminance,PM1,PM10,PM25,Temperature,VOC,PublishTime) values (null, %s,%s,%s,%s,%s,%s,%s,%s,%s)"
        data=SiteName,Humidity,Illuminance,PM1,PM10,PM25,Temperature,VOC,PublishTime
        cursor.execute(insert_sql,data)
        cursor.execute("""
           UPDATE pc_voc_rt
           SET Humidity=%s,Illuminance=%s,PM1=%s,PM10=%s,PM25=%s,Temperature=%s,VOC=%s,PublishTime=%s
           WHERE SiteName=%s
